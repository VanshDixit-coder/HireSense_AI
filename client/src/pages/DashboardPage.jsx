  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="glass-card p-6">
        <p className="text-sm font-semibold text-emerald-200">Welcome back, {user?.name}</p>
        <h1 className="mt-2 font-heading text-4xl font-bold">Your application cockpit</h1>
        <p className="mt-3 max-w-3xl text-white/65">Track opportunities, generate tailored materials, and keep your skill gaps visible.</p>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <div className="glass-card p-5">
          ...
          <p className="mt-2 font-heading text-4xl font-bold">{applications.length}</p>
        </div>
        <div className="glass-card p-5">
          <p className="text-sm text-white/55">Average Match Score</p>
          <p className="mt-2 font-heading text-4xl font-bold">{averageScore}%</p>
        </div>
        <div className="glass-card p-5">
          <p className="text-sm text-white/55">Saved Jobs</p>
          <p className="mt-2 font-heading text-4xl font-bold">{savedJobs.length}</p>
        </div>
      </div>
      ...
      <div className="mt-10">
        <div className="flex items-center justify-between gap-4">
          ...
          <Link to="/jobs" className="text-sm font-semibold text-blue-200">Browse all</Link>
        </div>
        <div className="mt-4 flex gap-4 overflow-x-auto pb-3">
          {recommendations.length ? recommendations.map(({ job, overlap }) => (
            <div key={job._id} className="w-80 shrink-0">
              <JobCard job={{ ...job, salaryRange: `${job.salaryRange} - ${overlap}% fit` }} compact />
            </div>
          )) : (
            <div className="glass-card w-full p-6 text-white/65">Add skills or upload a resume to unlock recommendations.</div>
          )}
        </div>
      </div>
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-3">
        <div className="glass-card overflow-hidden lg:col-span-2">
          <div className="border-b border-white/10 p-5">
            <h2 className="font-heading text-2xl font-bold">Recent Applications</h2>
          </div>
              ...
              <tbody>
                {applications.map((item) => (
                  <tr key={item._id} className="border-t border-white/10">
                    ...
                    <td className="px-5 py-4"><MatchScoreBadge score={item.matchScore} /></td>
                    <td className="px-5 py-4 text-white/60">{formatDate(item.appliedAt)}</td>
                    <td className="px-5 py-4">
                ...
                {!applications.length && (
                  <tr><td colSpan="4" className="px-5 py-8 text-center text-white/55">Analyze a job fit to start tracking applications.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <aside className="glass-card p-5">
          ...
          <div className="mt-4 space-y-3">
            {resources.length ? resources.map((item) => (
              <div key={item.skill} className="rounded-lg border border-white/10 bg-white/5 p-3">
                <p className="font-semibold text-white">{item.skill}</p>
                <p className="mt-1 text-sm text-white/60">{item.recommendedCourse}</p>
                <p className="mt-1 text-xs text-emerald-200">{item.platform} - {item.estimatedTime}</p>
              </div>
            )) : <p className="text-sm leading-6 text-white/60">Run a fit analysis to generate learning priorities.</p>}
          </div>
        </aside>
      </div>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 p-4">
          <div className="glass-card max-h-screen w-full max-w-3xl overflow-auto p-6">
            <div className="flex items-center justify-between">
              ...
              <button className="btn-secondary px-3 py-2" onClick={() => setModal(null)}>Close</button>
            </div>
            <pre className="mt-5 whitespace-pre-wrap rounded-lg bg-black/30 p-4 text-sm leading-6 text-white/80">{modal.text}</pre>
          </div>
        </div>
      )}
    </section>
